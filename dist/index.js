...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.artifactoryRepoName = void 0;
const core = __importStar(__nccwpck_require__(2186));
const artifactoryRepoName = (url) => {
    if (!url)
        return undefined;
    try {
        ...
        if (core.isDebug())
            core.debug(`failed to parse url '${url}': ${e}`);
        return undefined;
    }
};
exports.artifactoryRepoName = artifactoryRepoName;
...
exports.main = void 0;
const core = __importStar(__nccwpck_require__(2186));
const read_metadata_1 = __nccwpck_require__(9893);
const main = async () => {
    // Inputs
    ...
    // Call
    const apps = await (0, read_metadata_1.readMetadata)(dir, glob);
    // Fail when nothing has been found
    if (apps.length === 0) {
        throw new Error(`No files found on path ${dir} with pattern ${glob}`);
    }
    // Set outputs
    core.setOutput('files', apps.map(app => app.file));
...
const defaultGlob = (recursive) => recursive ? `**/${yamlPattern}` : yamlPattern;
...
const artifactory_repo_name_1 = __nccwpck_require__(4567);
const parsers = {
...
const readMetadata = async (cwd, patterns) => {
    // Find files
    ...
    const result = await Promise.all(files.map(async (f) => readApplication(cwd, f)));
    // Remove undefined entries
    const apps = result.filter((app) => !!app);
    core.info(`using files: ${apps.map(f => f?.file)}`);
    // Return
    return apps;
};
exports.readMetadata = readMetadata;
const readApplication = async (cwd, file) => {
    try {
        // Read and parse
        const content = await promises_1.default.readFile(node_path_1.default.resolve(cwd, file), {
            encoding: 'utf-8'
        });
...
function argoApplicationParser(file, data) {
    const app = {
        file,
        name: data.metadata?.name,
        repoURL: data.spec?.source?.repoURL,
        repoName: (0, artifactory_repo_name_1.artifactoryRepoName)(data.spec?.source?.repoURL),
...
function generalApplicationParser(file, data) {
    const app = {
        file,
        name: data.metadata?.name,
        repoURL: data.spec?.source?.repoURL,
